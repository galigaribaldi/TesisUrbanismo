% !TeX program = xelatex
% !TeX encoding = UTF-8
\documentclass[12pt,letterpaper,oneside]{book}

% =============================================================================
%  1. IDIOMA Y CODIFICACIÓN
%  babel debe ir primero: establece el idioma que todos los demás consultan
% =============================================================================
\usepackage[spanish,es-tabla,es-nodecimaldot]{babel}
\usepackage{csquotes}          % Habilita \blockquote y manejo de citas con babel

% =============================================================================
%  2. TIPOGRAFÍA Y LAYOUT BASE
% =============================================================================

\usepackage{geometry}
\geometry{top=2.5cm, bottom=2.5cm, left=3.0cm, right=2.5cm}
\usepackage{setspace}
\usepackage{emptypage}
\usepackage{microtype}         % Con XeLaTeX solo activa tracking; es normal

% =============================================================================
%  3. GRÁFICOS
% =============================================================================
\usepackage{graphicx}

% =============================================================================
%  4. MATEMÁTICAS
%  amsmath debe ir antes de hyperref: redefine internals que hyperref parchea
% =============================================================================
\usepackage{amsmath, amsfonts, amssymb}
\usepackage{bm}

% =============================================================================
%  5. ENCABEZADOS
% =============================================================================
\usepackage{fancyhdr}
\setlength{\headheight}{25.44003pt}
\addtolength{\topmargin}{-10.44003pt}

% =============================================================================
%  6. TÍTULOS DE CAPÍTULO
%  titlesec DEBE ir antes de hyperref.
%  bookmark corrige los bookmarks PDF que titlesec rompe.
% =============================================================================
\usepackage{titlesec}
\usepackage{bookmark}

% =============================================================================
%  7. BIBLIOGRAFÍA
%  natbib DEBE ir antes de hyperref
% =============================================================================
\usepackage{natbib}

% =============================================================================
%  8. COMANDOS Y PAQUETES ADICIONALES
%  Carga tablas, figuras, código, TikZ, colores y todos los \newcommand
% =============================================================================
\input{Latex/Comands}

% =============================================================================
%  9. HIPERVÍNCULOS Y PDF
%  hyperref VA AL FINAL del preámbulo; unamAzul ya está definido en Comands.tex
% =============================================================================
\usepackage{hyperref}
\hypersetup{
    pdftitle   = {Transportes Anillares y su Importancia en la Ciudad de Mexico},
    pdfauthor  = {Hernan Galileo Cabrera Garibaldi},
    pdfsubject = {Tesis de Maestria en Urbanismo, UNAM},
    pdfcreator = {XeLaTeX + natbib/bibtex},
    colorlinks = true,
    linkcolor  = unamAzul,
    citecolor  = unamAzul,
    urlcolor   = unamAzul,
    breaklinks = true,
    unicode    = true,
    pdfencoding= auto
}

% =============================================================================
%  CONFIGURACIÓN DE ENCABEZADOS (fancyhdr)
% =============================================================================
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\footnotesize\leftmark}
\fancyhead[R]{\thepage}
\renewcommand{\headrulewidth}{0.4pt}

\fancypagestyle{plain}{
  \fancyhf{}
  \fancyfoot[C]{\thepage}
  \renewcommand{\headrulewidth}{0pt}
}

% =============================================================================
%  HOOKS DE COMPILACIÓN SELECTIVA
%  Activar desde Makefile — nunca editar directamente.
% =============================================================================

% make borrador → figuras como caja vacía (compilación ~3× más rápida)
\ifdefined\borrador
  \PassOptionsToPackage{draft}{graphicx}
\fi

% make solo CAP=<ruta> → compila solo ese capítulo (p.ej. 6-Analisis.../1-Analisis)
\ifdefined\SoloCapitulo
  \includeonly{\SoloCapitulo}
\fi

% =============================================================================
%  INICIO DEL DOCUMENTO
% =============================================================================
\begin{document}

% ---------------------------------------------------------------------------
%  PORTADA
% ---------------------------------------------------------------------------
\begin{titlepage}
  \centering

  \begin{small}
  \begin{longtable}{@{} m{0.25\textwidth} @{} m{0.50\textwidth} @{} m{0.25\textwidth} @{}}
    \centering \vcenteredinclude{Logos/UNAM_crest_blue.pdf} &
    \centering
      \textbf{UNIVERSIDAD NACIONAL AUTÓNOMA DE MÉXICO} \\
      \vspace{0.2cm}
      \textbf{PROGRAMA DE MAESTRÍA Y DOCTORADO EN URBANISMO} \\
      \vspace{0.1cm}
      \textbf{FACULTAD DE ARQUITECTURA} &
    \centering \vcenteredinclude{Logos/escudo-fes-azul.png}
  \end{longtable}
  \end{small}

  \vspace{0.5cm}
  \rule{\textwidth}{1pt}
  \vspace{1.2cm}

  {\Large \textbf{Transportes Anillares y su Importancia en la Ciudad de México
  y el Área Metropolitana}} \\

  \vspace{2.0cm}

  {\large \textbf{TESIS}} \\
  \vspace{0.3cm}
  {QUE PARA OPTAR POR EL GRADO DE:} \\
  {\large \textbf{MAESTRO EN URBANISMO}} \\

  \vspace{1.5cm}

  {PRESENTA:} \\
  {\large \textbf{Hernán Galileo Cabrera Garibaldi}} \\

  \vspace{1.5cm}

  {TUTOR:} \\
  {\large \textbf{DR. DAVID LÓPEZ FLORES}} \\

  \vspace{1.2cm}

  {\small \textbf{MIEMBROS DEL COMITÉ TUTOR:}} \\
  \vspace{0.3cm}
  {\footnotesize
    1.~\rule{6cm}{0.4pt} \\
    2.~\rule{6cm}{0.4pt} \\
    3.~\rule{6cm}{0.4pt}
  } \\

  \vfill

  \rule{0.7\textwidth}{0.4pt} \\
  {\footnotesize UNIDAD DE POSGRADO DE LA FACULTAD DE ARQUITECTURA Y URBANISMO} \\
  \vspace{0.4cm}
  {\footnotesize Ciudad Universitaria, CDMX. \hfill 4 de febrero del 2026}
\end{titlepage}

% ---------------------------------------------------------------------------
%  PÁGINAS PRELIMINARES
% ---------------------------------------------------------------------------
\frontmatter
\onehalfspacing
\tableofcontents
\listoffigures

% ---------------------------------------------------------------------------
%  CUERPO PRINCIPAL
% ---------------------------------------------------------------------------
\mainmatter

\include{1-Introduccion/1-Introduccion}
\include{2-MarcoTeorico/2-MarcoTeorico}
\include{3-Conceptos-Indicadores/3-Conceptos}
\include{4-Capas-Codigo/4-Capas}
% --- Capítulos originales preservados (no eliminar) ---
% \include{5-Resultados/1-Resultados}
% \include{6-Analisis-SocioEspacial/1-Analisis}
% ------------------------------------------------------
\include{5-Analisis-Red-Actual/1-Analisis-Red-Actual}
\include{6-Analisis-Red-Anillo/1-Analisis-Red-Anillo}
\include{7-Conclusiones/1-Conclusiones}

% ---------------------------------------------------------------------------
%  ANEXOS
% ---------------------------------------------------------------------------
\appendix

\chapter{Documentación Técnica: Apimetro}
\label{anx:apimetro-tecnico}
\input{Anexos/A-Apimetro-Tecnico}

\chapter{Documentación Técnica: VFTModel}
\label{anx:vftmodel-tecnico}
\input{Anexos/B-VFTModel-Tecnico}

\chapter{Documentación Técnica: Transport-gis}
\label{anx:transportgis-tecnico}
\input{Anexos/C-TransportGis-Tecnico}

\chapter{Implementaciones Computacionales de Indicadores}
\label{anx:indicadores-implementacion}
\input{Anexos/D-Indicadores-Pendientes}

\chapter{Cartografía de Presentación: Transport-gis}
\label{anx:cartografia-presentacion}
\input{Anexos/E-Cartografia-Presentacion}

\chapter{Recursos Visuales Analíticos: Red Actual de la \protect\zmvm}
\label{anx:visualizaciones-analiticas}
\input{Anexos/F-Visualizaciones-Analiticas}

% ---------------------------------------------------------------------------
%  BIBLIOGRAFÍA
% ---------------------------------------------------------------------------
\backmatter
\bibliographystyle{apalike}
\bibliography{referencias}

\end{document}
