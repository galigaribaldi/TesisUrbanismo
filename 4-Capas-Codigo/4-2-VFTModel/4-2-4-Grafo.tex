...
% =============================================================================
\subsection{Construcción del Grafo Topológico}
\label{sec:vft-grafo}

\subsubsection{De la Geometría GeoJSON a NetworkX}
\label{subsec:vft-geom-networkx}

El módulo central de VFTModel es la clase \texttt{VFTGraphBuilder},
responsable de transformar las geometrías \geojson{} en un \textbf{dígrafo}
representado mediante la biblioteca \textbf{NetworkX} de Python
\citep{hagberg2008networkx}. La construcción opera en dos fases internas:
...
\begin{description}
    ...
    línea recta entre las estaciones—. Esta distinción resulta especialmente
    relevante en sistemas como \textsc{rtp} y Corredores Concesionados, cuyos
    trazos urbanos pueden superar con amplitud la cuerda directa entre paradas.
\end{description}

\subsubsection{Snapping Lógico Geográfico}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        ...
        \label{fig:snap3}
    \end{subfigure}
    \caption[Secuencia del Snapping Lógico Geográfico]{Representación visual
    del proceso de \termino{Snapping Lógico Geográfico}. Las estaciones
    cercanas de distintos sistemas se conectan mediante aristas de transbordo
    si cumplen con la tolerancia de distancia establecida.}
...
metropolitana, cuyas definiciones se presentan en el
Cuadro~\ref{tab:vft-umbrales}.
...
la implementación íntegra de \texttt{VFTGraphBuilder} —incluyendo la lógica
\termino{KDTree} y el cálculo de \termino{boarding cost} en los transbordos
peatonales— se documenta en el Anexo~\ref{anx:vft-graphbuilder}.
...
\begin{description}
    ...
    entre estaciones próximas de distintos sistemas. El costo de cada arista
    incluye el tiempo de caminata —a 5~km/h, según el límite superior establecido
    por \citet{sedatu2019manual}— más la espera promedio del siguiente servicio
    ($F_v / 2$). Este modo es el utilizado en todos los cálculos de indicadores
    presentados en el Capítulo~\ref{ch:resultados-vft}.
\end{description}

Con el grafo topológico construido y los modos de operación definidos, el
paso siguiente es asignar a cada arista un peso temporal mediante el motor
de impedancia.
